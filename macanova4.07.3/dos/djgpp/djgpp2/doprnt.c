...
/*
  Modifications by C. Bingham kb@stat.umn.edu 970407
  If FULLPREC is defined, this gives up to three leading zeros for
  g-format representation of numbers between .0001 and .1, that is, say
  using format "%12.5g" 1e-4*PI prints as .00031416, not as .00031

  Also, suitable #ifndefs have been inserted so that MAXEXP and
  MAXFRACT can be defined in a makefile.

  The file has been reformatted to more closely match the style used
  in MacAnova.
*/
...
#else /*FULLPREC*/
#define	BUF		(MAXEXP+MAXFRACT+4)	/* + decimal point + 3 possible extra 0's*/
#endif /*FULLPREC*/
	                ...
	                char *startp, char *endp)
{
	...
	switch (fmtch)
	{
	  ...
	  case 'G':
		...
		/*
n		 * if precision required or alternate flag set, add in a
...
#else /*FULLPREC*/
		/* don't count first 3 leading zeros as part of prec if there 
		   was no integer part
		   */
		{
			int     count = (expcnt) ? 3 : 0;
