...
/*
	Changes by kb@stat.umn.edu for use of readline with MacAnova are
	enabled by -DINPUTRCFIX  -DARROWKEYFIX on compile

	When INPUTRCFIX is defined the inputrc file is opened in binary mode
	and extraneous CR's filtered out.  If not done, then the file size
	info from stat does not match what is read.

	Defining ARROWKEYFIX enables code that let PC arrow keys work the way
	you would expect them to

	These changes would appear desirable for any MSDOS use of readline, not
	just MacAnova
*/
#endif
...
#ifdef INPUTRCFIX
  /*
	 When not opened in binary mode, the stat file length does not
	 match the number of bytes read in
  */
  if ((stat (openname, &finfo) < 0) ||
      (file = open (openname, O_RDONLY|O_BINARY, 0666)) < 0) /*KB 950919*/
#else /*INPUTRCFIX*/
  if ((stat (openname, &finfo) < 0) ||
...
#ifdef ARROWKEYFIX
/*
   rl_getc() has been hacked to translate MSDOS arrow keys to corresponding
   emacs movement key codes
*/
...
#ifdef __GO32__
#ifdef ARROWKEYFIX
/* fix to make arrow keys equivalent to emacs motion keys */
	if (isatty(0))
	{
		int c = getkey();
